...
function findLongestWord(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (currentWord.length > savedWord.length) {
...
/* let savedWord = inception */
console.log(findLongestWord(words));
/* 
  1. declare variable as savedWord with first spot in array
  2. start a forLoop to loop over the array words
  3. use an if statement to check the length of the string
  4. if its bigger save it in the variable savedWord
  5. after the if statement, return the saved word
  */
...
function averageWordLength(words) {
  ...
  return (sumOfCharacters / words.length)
}
...
function uniquifyArray(words) {
  // TODO:
  const arrayWithoutDuplicats = []

  for (let i = 0; i < words.length; i++) {
    const element = words[i];
    if (!arrayWithoutDuplicats.includes(words[i])) {
      arrayWithoutDuplicats
    } else {
